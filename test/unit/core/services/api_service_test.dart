...
void main() {
  group('APIService', () {
    ...
    test('get calls client.get with the correct URL', () async {
      ...
      when(mockClient.get(Uri.parse(baseUrl))).thenAnswer((_) async => http.Response(responseBody, 200));

      // Act
      await apiService.get();

      // Assert
      verify(mockClient.get(Uri.parse(baseUrl))).called(1);
    });
    ...
    test('get throws APIException when the http call completes with an error', () async {
      // Arrange
      when(mockClient.get(Uri.parse(baseUrl))).thenAnswer((_) async => http.Response('Error', 404));
